...
from util.process import find_view_from_cluster
...
class PoseTrack():
    ...
    def __init__(self, cameras):
        ...
        self.bank_size = 100
        self.feat_bank = np.zeros((self.bank_size, 2048)) # WHy 2048? 
        self.feat_count = 0
    ...
    def reactivate(self, newtrack):
        ...
        if newtrack.feat_count >= 1:
            ...
            # We take the new feature if the similarity is less than the threshold so we get NEW features
    ...
    def get_output(self):
        ...
        #if no singe-view feet, then choose bbox bottom point
        bottom_points = []
        for v in self.valid_views:
            bbox = self.bbox_mv[v]
    ...
    def multi_view_init(self, detection_sample_list, id):
        ...
        for sample in detection_sample_list:
            ...
            if all(sample.keypoints_2d[self.upper_body,-1] > 0.5) and sample.bbox[4] > 0.9 and np.sum(self.iou_mv[cam_id]>0.3)<2:
                if self.feat_count:
    ...
    def multi_view_3D_update(self, avail_tracks):
        ...
        for v in valid_views:
            #Check if there are misclassified ocluded state or state is not occluded in all cameras
            ...
            # if there are similarities more than 50% than it is not occluded
            if self_sim > 0.5:
                self.oc_idx[v] = []
                continue
            ...
            for t_id, track in enumerate(oc_tracks):
                #If there is no features in the current track
            ...
            self.oc_idx[v] = []
            #Switch view if there is a better view of the track available
    ...
    def CalcTargetRays(self, v):
        ...
        return aic_cpp.compute_joint_rays(self.keypoints_mv[v], cam.project_inv, cam.pos)
...
class PoseTracker():
    def __init__(self, cameras):
        ...
        self.thred_homo = 1.5
        self.thred_bbox = 0.4
        self.keypoints_thrd = 0.7
        self.glpk_bib = GLPKSolver(min_affinity=-1e5)
        self.main_joints = np.array([5,6,11,12,13,14,15,16])
        self.bank_size = 30
        self.thred_reid = 0.5
        self.upper_body = np.array([5,6,11,12])
            ...
            for s_id, sample in enumerate(detection_sample_list_mv[v]):
                ...
                for t_id, track in enumerate(avail_tracks):
                    ...
                    if reid_sim.size:
                        ...
                        reid_sim_sv[s_id,t_id] -= self.thred_reid
    ...
    def compute_3dkp_aff(self, detection_sample_list_mv, avail_tracks):
        ...
        for v in range(self.num_cam):
            ...
            cam = self.cameras[v]
            for s_id, sample in enumerate(detection_sample_list_mv[v]):
                joints_h = np.vstack((sample.keypoints_2d[:,:-1].T, np.ones((1, sample.num_keypoints))))
                joints_rays = cam.project_inv @ joints_h
                joints_rays /= joints_rays[-1]
                joints_rays = joints_rays[:-1]
                joints_rays -= np.repeat(cam.pos.reshape(3,1), sample.num_keypoints, axis=1)
                joints_rays = joints_rays / (np.linalg.norm(joints_rays, axis=0)+1e-5)
                joints_rays = joints_rays.T #17*3
                for t_id, track in enumerate(avail_tracks):
                    aff = np.zeros(sample.num_keypoints)
                    kp_3d = track.keypoints_3d
                    k_idx = np.where(sample.keypoints_2d[:,-1] < self.keypoints_thrd)[0]

                    aff[k_idx] = Point2LineDist(kp_3d[k_idx, :-1], cam.pos, joints_rays[k_idx])

                    valid = (sample.keypoints_2d[:,-1] > self.keypoints_thrd) * (kp_3d[:,-1] > 0)
        ...
        for v in range(self.num_cam):
            ...
            for s_id, sample in enumerate(detection_sample_list_mv[v]):
                ...
                for t_id, track in enumerate(avail_tracks):
                    ...
                    valid = (joints_t[:,-1] > self.keypoints_thrd) * (joints_s[:,-1] > self.keypoints_thrd)
    ...
    def compute_epi_homo_aff(self, detection_sample_list_mv, avail_tracks):
        ...
        for v in range(self.num_cam):
            ...
            for s_id, sample in enumerate(detection_sample_list_mv[v]):
                ...
                feet_valid_s = np.all(joints_s[feet_idxs,:-1] > self.keypoints_thrd)
                ...
                for t_id, track in enumerate(avail_tracks):
                    ...
                    aff_homo_sv = []
                    if feet_valid_s:
                        feet_valid_s = (joints_t[:, feet_idxs[0], -1] > self.keypoints_thrd) & (joints_t[:,feet_idxs[1], -1]> self.keypoints_thrd)
                    ...
                    for vj in range(self.num_cam):
                        ...
                        aff_temp = aic_cpp.epipolar_3d_score_norm(pos,sv_rays[s_id], pos_j, track_ryas_sv, self.thred_epi)
                        ...
                        if feet_valid_s and feet_valid_t[vj]:
                            ...
                            aff_homo_ss.appenf(_aff_homo_ss)
    ...
    def compute_bboxiou_aff(self, detection_sample_list_mv, avail_tracks):
        ...
        for v in range(self.num_cam):
            ...
            multi_covarience = np.stack([track.bbox_kalman[v].covariance.copy() if track.bbox_kalman[v].covariance is not None else np.eye(8) for track in avail_tracks])
            multi_mean, multi_covarience = avail_tracks[0].bbox_kalman[v].multi_predict(multi_mean, multi_covarience)
            for i, (mean, cov) in enumerate(zip(multi_mean, multi_covarience)):
            ...
            iou = (((iou-0) * (age<=15)).T * score).T #change IoU threshold from 0.5 to 0.2 to 0
            aff_mv.append(iou)
    ...
    def match_with_miss_tracks(self, new_track, miss_tracks):
        if len(miss_tracks) == 0:
            ...
            print("new_init", new_track.id)
            return
        
        reid_sim = np.zeros(len(miss_tracks))
        for t_id, track in enumerate(miss_tracks):
            if track.feat_count == 0 or new_track.feat_count:
                continue
        ...
        if reid_sim[t_id]>0.5:
            ...
            print("reactivate", miss_tracks[t_id].id, new_track.valid_views)
    ...
    def target_init(self, detection_sample_list_mv, miss_tracks, iou_det_mv, ovr_det_mv, ovr_tgt_mv):
        ...
        for vi in range(self.num_cam):
            sample_vi = detection_sample_list_mv[vi]
            ...
            for vj in range(self.num_cam):
                if vi==vj:
                    ...
                    pos_j = self.cameras[vj].pos
                    sample_vj = detection_sample_list_mv[vj]
                    ...
                    for a in range(det_count[vi]):
                        sample_a = sample_vi[a]
                        feet_valid_a = np.all(sample_a.keypoints_2d[feet_idxs,-1]>self.keypoints_thrd)
                        if feet_valid_a:
                            ...
                            feet_a = np.array([(sample_a.bbox[0]+sample_a.bbox[0])/2, sample_a.bbox[3]]) ####### Look further into
                        ...
                        for b in range(det_count[vj]):
                            sample_b = sample_vj[b]
                            aff = np.zeros(self.num_keypoints)
                            valid_kp = (sample_a.keypoints_2d[:,-1]> self.keypoints_thrd) & (sample_b.keypoints_2d[:,-1]>self.keypoints_thrd)
                            j_id = np.where(valid_kp)[0]
                            aff[j_id] = epipolar_3d_score_norm(pos_i, mv_rays[vi][a][j_id,:], pos_j, mv_rays[vj][b][j_id,:], self.thred_epi)

                            if feet_valid_a and np.all(sample_b.keypoints_2d[feet_idxs, -1]>self.keypoints_thrd):
        ...
        clusters, sol_matrix = self.glpk_bib.solve(aff_final, True)

        for cluster in clusters:
            if len(cluster) == 1:

                view_list, number_list = find_view_from_cluster(cluster, det_all_count)
    ...
    def ltrb2xyah(self, ret):
        ...
        center_x = ret[..., 0] + width / 2
        center_y = ret[..., 1] + height / 2
        
        # Calculate aspect_ratio
        aspect_ratio = width / height
        
        # Combine the results into the output array
        ret[..., 0] = center_x
        ret[..., 1] = center_y
        ret[..., 2] = aspect_ratio
        ret[..., 3] = height
    ...
    def mv_update_wo_pred(self, detection_sample_list_mv, frame_id = None):
        ...
        avail_tracks = [track for track in self.tracks if track.state < TrackState.Missing] #Confirmed and Unconfirmed
        ...
        aff_2dkp = self.compute_2dkp_aff(detection_sample_list_mv, avail_tracks)
        ...
        for v in range(self.num_cam):
            ...
            aff_epi[v][aff_epi[v] < (-a_box+0.5*a_box)] = -a_box+0.50*a_box
            aff_homo[v][aff_homo[v] < (-a_box+0.5*a_box)] = -a_box+0.5*a_box

            norm = a_epi *(aff_epi[v] != 0).astype(float) + a_box * (aff_box[v]!=0).astype(float) + a_homo *(aff_homo[v] !=0).astype(float) + a_2dkp * (aff_2dkp[v] != 0).astype(float)
            aff_final = (a_epi*aff_epi[v] + a_box*aff_box[v] + a_homo*aff_homo[v] + aff_reid[v]*a_reid*reid_weight[v] + a_2dkp*aff_2dkp[v])/(1+reid_weight[v])

            idx = np.where(norm>0)
            aff_final[idx] -= (a_box-norm[idx]) * 0.1
            sample_list_mv = detection_sample_list_mv[v]
            ...
            for row, col in zip(row_idxs, col_idxs):
                ...
                if True:
                    avail_tracks[col].single_view_2D_update(v, sample_list_mv[row], iou, ovr, ovr_tgt, avail_idx)
                    updated_tracks.add(col)
                    matched_det_sv.add(row)

            unmatched_det_sv = list(set(range(len(sample_list_mv))) - matched_det_sv)
            unmatched_sv = [sample_list_mv[u] for u in unmatched_det_sv]
            ...
            if track.state == TrackState.Confirmed:
                if track.update_age==1:
                    for v in track.valid_views:
                        ...
                        record = np.array([[self.cameras[v].idx_int, track.id, frame_id, bbox[0], bbox[1], bbox[2]-bbox[0], bbox[3]-bbox[1], track.output_cord[0], track.output_cord[2]]])
